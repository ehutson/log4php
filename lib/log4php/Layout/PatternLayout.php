...
namespace log4php\Layout;

use log4php\LoggerLayout,
    \log4php\LoggerLoggingEvent,
    \log4php\Helper\PatternParser,
 ...
 * @since 0.3 
 */
class PatternLayout extends LoggerLayout
{
    ...
    public function setConversionPattern($conversionPattern)
    {
        ...
        $patternParser = new PatternParser($this->pattern);
    ...
    public function formatToArray(LoggerLoggingEvent $event)
    {
        ...
        $c = $this->head;
        while ($c !== null) {
            if (!$c instanceOf LiteralPatternConverter) {
                $sbuf      = null;
                $c->format($sbuf, $event);
                $results[] = $sbuf;
            }
