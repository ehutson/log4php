...
namespace log4php\Configurator;

use \log4php\LoggerException,
    \log4php\LoggerConfigurator,
    \log4php\LoggerHierarchy,
    \log4php\Renderer\ObjectRenderer,
    \log4php\LoggerAppender,
    \log4php\LoggerLevel,
    \log4php\LoggerLayout,
    \log4php\LoggerFilter,
    \log4php\Logger,
    \log4php\Helper\OptionConverter;
 ...
 * @since 2.2
 */
class DefaultAdapter implements LoggerConfigurator
{
    ...
    private function doConfigure(LoggerHierarchy $hierarchy, $config)
    {
        ...
        if (isset($config['loggers']) && is_array($config['loggers'])) {
            foreach ($config['loggers'] as $loggerName => $loggerConfig) {
                $this->configureOtherLogger($hierarchy, $loggerName,
                                            $loggerConfig);
            }
        }
    ...
    private function configureRenderer(LoggerHierarchy $hierarchy, $config)
    {
        ...
        if (!$renderingClassInstance instanceof ObjectRenderer) {
    ...
    private function configureLogger(Logger $logger, $config)
    {
        ...
        if (isset($config['additivity'])) {
            $additivity = OptionConverter::toBoolean($config['additivity'], null);
